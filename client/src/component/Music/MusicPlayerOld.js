...
const MusicPlayerOld = () => {
  const audioUrls = ["https://ex/song1.mp3", "https://xe/song2.mp3"];
  const [playlist, setPlaylist] = useState(["제목1", "제목2", "제목3"]); // 플레이 리스트
  const [currentSong, setCurrentSong] = useState(0); // 현재 재생중인 음악
  const [isPlaying, setIsPlaying] = useState(false); // 음악 재생, 음악 일시정지를 위한 상태
  const [audio, setAudio] = useState(new Audio(audioUrls[currentSong])); // 오디오박스
  ...
  return (
    <div>
      ...
      <button onClick={plyaNextSong}>다음 곡 재생</button>
      <button onClick={displayPlayList}>재생 목록</button>
    </div>
  );
};
